...
const insertCat = async (cat) => {
	try {
		const [row] = await promisePool.execute('INSERT INTO wop_cat (name, weight, owner, filename, birthdate, coords) VALUES (?, ?, ?, ?, ?, ?)', 
			[cat.name, cat.weight, cat.owner , cat.filename, cat.birthdate, cat.coords]);
